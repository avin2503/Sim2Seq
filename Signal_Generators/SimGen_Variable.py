...
def index_where_threshold_crossed(arr, threshold):
    total = 0
    for idx, val in enumerate(arr):
        if total + val > threshold:
            return idx - 1  
        total += val
    return len(arr) - 1  
...
def predict_DNA_6mer_5_3_with_sampling(template, lut, lambda_time, sampling_rate=5000, I_max=180):
    ...
    backstep_prob = 0.005 # probability of backstep
    skip_prob = 0.01  # probability of forward skip
    max_skip = 2
    max_backstep = 2
    ...
    while i < N:
        kmer = kmers[i]
        step_time = max(np.random.exponential(scale=lambda_time), min_step_time) 
        ns = int(step_time * sampling_rate)  
        if ns == 0:
            i += 1
            continue
        bases_per_point.append(1 / (2 * ns))
        params = lut[kmer]
        ...
        drift = drift_per_second * (current_time / 1.0)
        ...
        smooth_noise = np.convolve(np.random.normal(0, 0.01 * I_max, ns + smoothing_window), 
                                   np.ones(smoothing_window) / smoothing_window, mode='valid')[:ns]
...
def extract_random_sequences(reference_genome_file, num_sequences, seq_length=3000):
...
def generate_random_signals(reference_genome_file, num_reads, seq_length=3000):
...
def align_references(signal_per_base, dna_sequence, signal_length, chunksize, overlap):
    ...
    raw_references = []
    reference_lengths = []
    cumulative_signal = np.cumsum(signal_per_base)
    for i in range(0, signal_length - chunksize + 1, step):
        signal_chunk_start = i
        signal_chunk_end = i + chunksize
        base_start = np.searchsorted(cumulative_signal, signal_chunk_start, side='left')
        base_end = np.searchsorted(cumulative_signal, signal_chunk_end, side='right')
        chunk_ref = dna_encoded[base_start:base_end] 
...
def process_batch_in_parallel(batch_seqs, lut, lambda_time, sampling_rate, output_dir, batch_idx):
    ...
    for seq_i, seq in enumerate(batch_seqs):
        signal_df,bases_per_point_list = predict_DNA_6mer_5_3_with_sampling(seq, lut, lambda_time, sampling_rate)
        signal = signal_df['current'].values
        sig_len = len(signal)
        dna_len = len(seq)
        signal_per_base = 1/np.array(bases_per_point_list)
     
        chunksize, overlap = 10000, 500
        ...
        num_retained_6mers=index_where_threshold_crossed(signal_per_base,retained_signal_length)
        signal_per_base = signal_per_base[len(signal_per_base)-num_retained_6mers-1:] 
        trimmed_seq_length = num_retained_6mers 
        trimmed_seq = seq[-trimmed_seq_length:]
        refs, ref_lens = align_references(signal_per_base, trimmed_seq, len(trimmed_signal), chunksize, overlap)
